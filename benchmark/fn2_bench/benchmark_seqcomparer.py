...
for record in SeqIO.parse(args.multifasta_filename, "fasta"):
            ...
            rc = sc.compress(seq)
            dist2reference = len(rc['A'])+len(rc['C'])+len(rc['T'])+len(rc['G'])
...
mem_profile_output_file = args.multifasta_filename+".memprofile_{0}.csv".format(args.snpCeiling)
...
all_sample_ids = sc.guidscachedinram()

if len(all_sample_ids) > 100:
    sample_ids = random.choice(all_sample_ids, 100)
else:
...
for sample_id in sample_ids:
    ...
    for cf_sample_id in all_sample_ids:
        res = sc.countDifferences_byKey((sample_id, cf_sample_id), cutoff=args.snpCeiling)
    t1 = datetime.datetime.now()
    delta = (t1 - t0).total_seconds()
    perflog_records.append(
        dict(
            start_time = t0, end_time = t1, delta_seconds = delta, sample_id = sample_id
        )
    )

perf_profile = pd.DataFrame.from_records(perflog_records, index='sample_id')
perf_profile = perf_profile.merge(mem_profile[['dist2reference','ns']], right_index = True, left_index= True)
perf_profile_output_file = args.multifasta_filename+".perfprofile_{0}.csv".format(args.snpCeiling)
